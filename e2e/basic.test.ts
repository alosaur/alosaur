...
test({
    ...
    async fn(): Promise<void> {
        ...
        try {
            const r1 = await fetchWithClose('http://0.0.0.0/home/query-name');
            const r2 = await fetchWithClose('http://0.0.0.0/home/query-name/');
            const r3 = await fetchWithClose('http://0.0.0.0/');
...
test({
    ...
    async fn(): Promise<void> {
        ...
        try {
            const response = await fetch('http://0.0.0.0/home/query-name?name=john');
